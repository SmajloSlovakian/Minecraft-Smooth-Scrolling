...
public class AnimatiMC implements ModInitializer {
    public static final Logger LOGGER = LoggerFactory.getLogger("animatimc");
    ...
    public static float creativeScreenGetPos(int row,CreativeScreenHandler that){
        return MathHelper.clamp((float)row / ((float)MathHelper.ceilDiv(AnimatiMC.creativeSH.itemList.size(), 9) - 5), 0.0F, 1.0F);
    }
}
